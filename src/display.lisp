...
(defun run-fullscreen-editor (buf &optional file)
  ...
  (charms:with-curses ()
    ...
    (let* ((win charms:*standard-window*)
           ...
           (message nil))
      (loop
        (format *error-output* "LOOP mode=~a buf-lines=~s~%" mode (buffer-lines buf))
        (draw-editor win buf mode message)
        (setf message nil)
        (let ((key (charms:get-char win :ignore-error t)))
          (format *error-output* "KEY: ~s mode-was: ~a~%" key mode)
          (if (eq mode :ex)
              ...
              (multiple-value-bind (nb nm msg)
                ...
                (format *error-output* "PRE-SETF nm=~s~%" nm)
                (setf buf nb mode nm message msg)
                (format *error-output* "POST-SETF mode=~a~%" mode)
